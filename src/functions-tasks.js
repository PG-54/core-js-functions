...
function getPolynom(...coefficients) {
  ...
  return (x) => {
    return coefficients.reduce((acc, c, idx) => {
      const pow = coefficients.length - 1 - idx;
      return acc + c * x ** pow;
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const logStr = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
...
function partialUsingArguments(fn, ...args) {
  ...
  return (...rest) => fn(...[...appliedArgs, ...rest]);
}
